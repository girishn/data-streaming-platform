locals {
  ...
  # When single_nat_gateway=true all private route tables point at the single NAT in AZ[0].
...
resource "aws_route_table" "public" {
  ...
  tags = merge(local.common_tags, { Name = "${var.environment_name}-public-rt" })
}
...
# One private route table per AZ — each points at its own NAT (or the single shared NAT).
